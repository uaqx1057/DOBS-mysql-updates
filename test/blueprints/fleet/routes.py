...
def dashboard_fleet():
    ...
    onboarding_drivers = (
        Driver.query
        .options(joinedload(Driver.platforms))  # <-- eager load platforms
    ...
    template = (
        "rtl_dashboard_fleet.html" if lang == "ar" else "dashboard_fleet.html"
    )
...
@login_required
def change_password():
    """Allow Fleet Manager to change password securely and send email notification."""
    if current_user.role != "FleetManager":
        flash("Access denied. Fleet Manager role required.", "danger")
        return redirect(url_for("auth.login"))
