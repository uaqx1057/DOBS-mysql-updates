...
def dashboard_finance():
    ...
    pending_drivers = (
        Driver.query
        .filter_by(onboarding_stage="Finance")
        .all()
    )
    ...
    offboarding_requests = (
        Offboarding.query
        .join(Offboarding.driver)  # ensure the driver exists
        .filter(Offboarding.status == "Finance")
        .order_by(Offboarding.requested_at.desc())
        .all()
    )

        # Count of drivers in onboarding (Finance stage)
    total_drivers = Driver.query.filter_by(onboarding_stage="Finance").count()

    # Count of drivers in offboarding (Finance stage)
    total_users = Offboarding.query.filter_by(status="Finance").count()
    # --- Select template based on language ---
    template = (
        "rtl_dashboard_finance.html" if lang == "ar" else "dashboard_finance.html"
    )
...
def approve_driver(driver_id):
    ...
    try:
        ...
        if recipients:
            ...
            Please complete the sponsorship transfer and finalize onboarding.
            
            Summary:
            - Transfer Fee: {"✅ Paid" if driver.transfer_fee_paid else "❌ Not Paid"}
            - Amount: {driver.transfer_fee_amount if driver.transfer_fee_amount is not None else "N/A"}
            - Paid At: {driver.transfer_fee_paid_at.strftime('%Y-%m-%d %H:%M') if driver.transfer_fee_paid_at else "N/A"}
            - Vehicle: {driver.car_details if driver.car_details else "N/A"}
            - TAMM Authorized: {"✅ Yes" if driver.tamm_authorized else "❌ No"}
            - Finance Cleared: {driver.finance_approved_at.strftime('%Y-%m-%d %H:%M') if driver.finance_approved_at else "N/A"}
            
            ------------------------------------------------------
            
            مرحباً فريق العمل،
            
            تمت الموافقة على السائق {driver.full_name} من قبل قسم المالية.
            يرجى إكمال نقل الكفالة والانتهاء من إجراءات الانضمام.
            
            الملخص:
            - رسوم النقل: {"✅ مدفوعة" if driver.transfer_fee_paid else "❌ غير مدفوعة"}
            - المبلغ: {driver.transfer_fee_amount if driver.transfer_fee_amount is not None else "N/A"}
            - تاريخ الدفع: {driver.transfer_fee_paid_at.strftime('%Y-%m-%d %H:%M') if driver.transfer_fee_paid_at else "N/A"}
            - المركبة: {driver.car_details if driver.car_details else "N/A"}
            - TAMM مصرح به: {"✅ نعم" if driver.tamm_authorized else "❌ لا"}
            - المالية تم اعتمادها: {driver.finance_approved_at.strftime('%Y-%m-%d %H:%M') if driver.finance_approved_at else "N/A"}
...
@login_required
def change_password():
    """Allow Finance Manager to change their own password securely and send email notification."""
    if current_user.role != "FinanceManager":
        flash("Access denied. Finance Manager role required.", "danger")
        return redirect(url_for("auth.login"))
    ...
    try:
        ...
        try:
            ...
            if emails:
                ...
                Driver {record.driver.full_name} (Iqama: {record.driver.iqama_number or "N/A"}) 
                ...
                فريق الموارد البشرية المحترم،
                
                تمت الموافقة المالية للسائق {record.driver.full_name} (رقم الإقامة: {record.driver.iqama_number or "N/A"}).
                
                ملاحظات المالية: {record.finance_note or "N/A"}
                التعديلات: {record.finance_adjustments or "لا يوجد"}
                
                يرجى المضي قدماً في الإجراءات النهائية.
        ...
        # ✅ Flash success message and refresh page
        flash(f"✅ Driver {record.driver.full_name} has been sent to HR successfully!", "success")
        return redirect(url_for("finance.dashboard_finance"))
