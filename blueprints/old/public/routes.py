import os
from flask import Blueprint, render_template, request, redirect, url_for, flash, session, send_from_directory, make_response, current_app
...
def register():
    if request.method == "POST":
        ...
        try:
            ...
            # Add manual fallback email
            #if "uaqx1057@gmail.com" not in recipients:
               # recipients.append("uaqx1057@gmail.com")
            if recipients:
                subject = "New Driver Registration Submitted / تسجيل سائق جديد"
                body_lines = [
                    "Dear Team,",
                    "",
                    f"Driver {full_name} has submitted a new registration form. Please review the details.",
                    "You can now log in to your account using the provided credentials on https://dobs.dobs.cloud/login.",
                    "------------------------------------------------------",
                    "",
                    "الفريق المحترم،",
                    "",
                    f"لقد قام السائق {full_name} بتقديم نموذج تسجيل جديد. يرجى مراجعة التفاصيل.",
                    "https://dobs.dobs.cloud/login يمكنك الآن تسجيل الدخول باستخدام بيانات الدخول أعلاه.",
                    "Regards / مع التحية,",
