    ...
    try:
        ...
        if recipients:
            msg = Message(
                ...
                recipients=recipients,
                body=f"""
                Driver {driver.full_name} has completed onboarding.
                Iqama: {driver.iqama_number or 'N/A'}
                Qiwa Contract: {'No' if not driver.qiwa_contract_created else 'Yes'}
                Status: Completed at {driver.sponsorship_transfer_completed_at.strftime('%Y-%m-%d %H:%M')}
                """
            )
            mail.send(msg)
