...
from models import Driver, Offboarding, BusinessID
...
def _format_platform_assignments(driver):
    ...
    active_history = [
        row for row in (getattr(driver, "driver_business_history", []) or [])
        if getattr(row, "transferred_at", None) is None
    ]
    business_id_ids = [row.business_id_id for row in active_history if getattr(row, "business_id_id", None)]
